...
export async function registerHandler(
  ...
  try {
    ...
    if (!user) {
      throw new HandlerError("AUTH_ERROR", "Error al registrar", 400);
    }
    ...
    // Envío de correo de bienvenida (no afecta el registro si falla)
    try {
      await sendWelcomeEmail({
        email,
        companyName: pyme.name,
      });
    } catch (error) {
    ...
    if (err.code === "23505") {
      throw new HandlerError(
        "DB_ERROR",
        "El nombre de la PYME ya esta en uso",
        400
      );
    }
